...
local RepositoryManager = Class.new(function(class)
    ...
    function self:Constructor()
        self.repositories = {};
        GM.Log:debug("[ RepositoryManager ] initialized.");
    end
    ...
    function self:Register(name, Object)
        if (not self.repositories[name]) then
            ...
            GM.Log:warn("RepositoryManager:register(): repository [ ".. name .." ] already exists");
            return nil;
        end
    end
    ...
    function self:Find(name, id)
        if (self.repositories[name]) then
            ...
            GM.Log:warn("RepositoryManager:find(): repository [ ".. name .." ] not found");
            return nil;
        end
    end
