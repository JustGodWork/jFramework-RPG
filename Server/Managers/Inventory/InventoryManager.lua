...
InventoryManager = Class.new(function(class)
    ...
    function self:Constructor()
        ...
        GM.Log:debug("[ InventoryManager ] initialized.");
            ...
            for _, row in pairs(result) do
                if (row) then
                    if (row.shared == 1) then
                        ...
                        GM.Log:debug("[ InventoryManager ] Loaded inventory " .. row.name .. " with id " .. row.id);
                    end
                end
            end
        end)
    ...
    function self:Register(id, name, label, owner, maxWeight, slots, shared)
        if (owner) then
            ...
            if (not self.owned[owner][id]) then
                ...
                GM.Log:warn("InventoryManager:register(): inventory [ ".. name .." ] already exists");
                return false;
            end
        else
    ...
    function self:GetByOwner(owner, inventoryName)
        ...
        if (self.owned[owner]) then
            ...
            GM.Log:warn('InventoryManager:getByOwner(): owner [ '.. owner ..' ] not found');
            return nil;
        end
        if (not found) then
            GM.Log:warn('InventoryManager:getByOwner(): inventory [ Owner: "'.. owner.. '" name: "'.. inventoryName ..'" ] not found');
            return nil;
        end
    end
    ...
    function self:SetOwner(inventory, owner)
        if (inventory) then
            ...
            elseif (self.owned[inventory:GetOwner()][inventory:GetId()]) then
                ...
                GM.Log:warn('InventoryManager:setOwner(): inventory [ '.. inventory ..' ] not found');
            end
        end
    end
