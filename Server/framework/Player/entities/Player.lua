...
function Player:OnConnect(data)
    ...
    GM.Server.log:debug("[ Player: ".. self:GetFullName() .." ] initialized.");
end
...
function Player:HandleRespawn()
    ...
    Timer.SetTimeout(function()
        if (playerCharacter and playerCharacter:IsValid()) then
            GM.Server.log:info(string.format("Respawning Player [%s] %s...", self:GetSteamID(), self:GetFullName()));
            ...
            GM.Server.log:warn(string.format("Player [%s] %s character is not valid, cannot respawn.", self:GetSteamID(), self:GetFullName()));
        end
