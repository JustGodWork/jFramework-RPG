...
Inventory = Class.new(function(class)
    ...
    function self:Constructor(id, name, label, owner, maxWeight, slots, shared)
        ...
        GM.Server.log:debug(("[ Inventory: %s ] initialized."):format(self.id));
    end
            ...
            if (#items > 0) then
                for _, item in pairs(items) do
                    ...
                    if (managedItem) then
                        ...
                        GM.Server.log:warn("Invalid Item [" .. item.name .. "] has been deleted from inventory " .. self.id .. ".");
                    end
                end
                self:UpdateWeight();
                GM.Server.log:debug("Inventory " .. self.id .. " has been built.");
            end
        end);
    end
