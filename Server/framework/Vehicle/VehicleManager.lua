...
---@type VehicleManager
local VehicleManager = Class.new(function(class)
    ...
    function self:Constructor()
        self.vehicles = {}
        GM.Server.log:debug("[ VehicleManager ] initialized.");
    end
    ...
    function self:Create(modelName, location, rotation)
        if (not self:ModelExist(modelName)) then
            GM.Server.log:warn("VehicleManager:create(): The model ["..modelName.."] does not exist.")
            return
        end
    ...
    function self:DeleteById(id)
        if (self.vehicles[id]) then
            ...
            GM.Server.log:warn("VehicleManager:deleteById(): Vehicle [".. id .."] not found")
            return false
        end
    end
...
---@type VehicleManager
GM.Server.vehicleManager = VehicleManager();
